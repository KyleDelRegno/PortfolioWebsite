...
export class ProjectData {
    ...
    constructor(project: string, type: string, role: string[], skills: string[], date: string, description: string, cover: string, Images: string[]){
...
export default function GridComponent({ onSelect }: GridDisplayData) {
    ...
    return (
        <div className="p-10 h-full grid gap-2
        grid-cols-2
        md:grid-cols-3
        xl:grid-cols-4">
            {items.map((project) =>( //loops through all the projects
                <div key={project.project} className={project.type === 'Video' ? 'row-span-1' : 'row-span-2'}>
                    ...
                    transition duration-300 ease-in-out hover:scale-120 hover:z-60" onClick={() => onSelect(project.project)}>
                        ...
                        <img src={project.cover} className={project.type === 'Video' ? "object-cover aspect-16/9" : "object-cover aspect-16/18"}></img>
                    </div>
                </div>
            ))}
        </div>
